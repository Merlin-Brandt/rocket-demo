...
impl<'r> FromRequest<'r> for User {
    ...
    async fn from_request(request: &'r Request<'_>) -> request::Outcome<User, Self::Error> {
        request.cookies()
            ...
            .map(|cookie| cookie.value().to_string()).map(User)
            .or_forward(())
    }
}
...
#[get("/")]
fn index(user: User) -> Html<String> {
    Html(String::new() + r#"
        <!DOCTYPE html>
        <html>
        <head>
...
#[get("/login", rank = 2)]
fn login_page(flash: Option<FlashMessage<'_>>) -> Html<String> {

    let (kind, message) = flash.map(|flash| flash.into_inner()).unwrap_or(("".to_string(), "".to_string()));

    Html(String::new() + r#"<!DOCTYPE html>
        <html>
        <head>
            <meta charset="utf-8" />
            <meta name="viewport" content="width=device-width" />
...
fn logout(jar: &CookieJar<'_>) -> Flash<Redirect> {
    jar.remove_private(Cookie::named("user_name"));
